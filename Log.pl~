...
sintagma_verbal4(S0,S):- verbo6(que,S0,S1),
						 aux6(S1, S2),
						 verbo5(S2,S3),
						 aux5(S3, S).
...
consulta6(X, Y, Z):-viajar(X, Z, Y, [], 0, R, C),
...
valida_lugar([X|A], B):- (lugar(X) -> B = X).
valida_lugar([X|A], B):- valida_lugar(A, B, X).

valida_lugar([X|A], B, C):- (lugar(X) -> B = X).
valida_lugar([X|A], B, C):- valida_lugar(A, B, X).
...
valida_lugares_iguales(X, Y, Z, X):-vuelve_a_empezar(2).
valida_lugares_iguales(X, Y, Z, Y):-vuelve_a_empezar(2).
valida_lugares_iguales(X, Y, Z, A):-(es_miembro(Z, A) -> vuelve_a_empezar(2)).
...
valida_si_no(["si"|A], X, Y, Z):-consulta5(X, Y, Z).
valida_si_no(["no"|A], X, Y, Z):-consulta6(X, Y, Z).
valida_si_no(["ninguno"|A], X, Y, Z):-consulta6(X, Y, Z).
...
es_miembro([X|A], X).
es_miembro([X|A], B):- es_miembro(A, B).
